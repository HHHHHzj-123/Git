...
root=Path(__file__).resolve().parent;out=root/'qa'/'contacts';out.mkdir(parents=True,exist_ok=True)
def make(files,name,cols=3,w=420):
 ...
 can.save(out/name)
docs=sorted((root/'qa'/'doc_render').glob('page-*.png'),key=lambda p:int(p.stem.split('-')[-1]))
