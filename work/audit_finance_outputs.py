...
for ep in (2, 3, 4):
    docx_path = ROOT / f"齐昊老师_学员实战案例_第{ep}集_文字思维导图.docx"
    ...
    xmind_path = ROOT / f"齐昊老师_学员实战案例_第{ep}集_思维导图.xmind"
    with zipfile.ZipFile(xmind_path) as archive:
        assert archive.testzip() is None
        content = json.loads(archive.read("content.json"))
        assert len(content) == 2
        assert {item["title"] for item in content} == {"01 总览图", "02 详细知识树"}
    print(f"EP{ep}\theadings={len(headings)}\tlists={len(numbered)}\ttables={len(doc.tables)}\tsheets={len(content)}")
